...
| `{{$key}}`
| {{$input.Description | adocCell}}
| {{if $input.Required}}✓{{else}}✗{{end}}
| {{if hasDefault $input.Default}}{{$input.Default | adocCode}}{{else}}_none_{{end}}
...
{{if hasDefault $input.Default}}Default:: {{$input.Default | adocCode}}{{end}}
